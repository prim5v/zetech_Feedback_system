...
const IssueDetailsPage = () => {
  ...
  useEffect(() => {
    const fetchIssueDetails = async () => {
      try {
        setLoading(true);
        const res = await fetch(`${API_BASE}/get_issue_details/${id}`);
        ...
        if (res.ok) {
          ...
          const mappedIssue = {
            ...
            status: issueData.status ? issueData.status.charAt(0).toUpperCase() + issueData.status.slice(1) : "Pending",
          ...
          console.error("Failed to fetch issue:", data.error || data.message);
        }
      } catch (err) {
  ...
  const handleSubmitResponse = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/post_response_to_issue`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          issue_id: issue.id,
      ...
      if (res.ok) {
        ...
        setResponseText('');
        setError('');
        setIssue({ ...issue, updatedAt: new Date() }); // ✅ update timestamp in UI
      } else {
  ...
  if (loading) {
    return <div className="max-w-3xl mx-auto text-center py-12">Loading issue details...</div>;
  }
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <button
        ...
        className="flex items-center text-zetech-blue hover:text-zetech-blue-dark transition-all mb-6"
      >
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
            <div className="bg-zetech-gray p-4 rounded-md">
              ...
              <p><TagIcon size={16} className="inline mr-2 text-gray-500" /> <b>Category:</b> {issue.category}</p>
              <p><CalendarIcon size={16} className="inline mr-2 text-gray-500" /> <b>Submitted:</b> {formatDate(issue.createdAt)}</p>
            ...
            <div className="bg-zetech-gray p-4 rounded-md">
              ...
              {issue.isAnonymous ? (
                <p className="italic text-gray-700"><UserIcon size={16} className="inline mr-2" /> Anonymous submission</p>
              ) : (
                <>
                  <p><UserIcon size={16} className="inline mr-2 text-gray-500" /> <b>Name:</b> {issue.studentName}</p>
                  <p><MailIcon size={16} className="inline mr-2 text-gray-500" /> <b>Email:</b> {issue.studentEmail}</p>
          ...
          <div className="border-t border-gray-200 pt-6 mt-6">
            ...
            <form onSubmit={handleSubmitResponse}>
              ...
              {error && <p className="text-red-500 text-sm mt-1 flex items-center">
                <AlertCircleIcon size={16} className="mr-1" /> {error}
              </p>}
